...
def main():
    ...
    train_loss = np.empty((1, 0))
    valid_loss = np.empty((1, 0))
    ...
    for epoch in range(args.epoch_start, args.num_epoch):
        ...
        # train and validate
        train_loss_ep = train(data_class, prob, model, optimizer, epoch, device)
        valid_loss_ep, ber = validate(data_class, prob, channel_dict, dummy_dict_start, 
                                      dummy_dict_end_eval, model, epoch, device)
        
        result.write('epoch %d \n' % epoch)
        result.write('information prob.'+str(prob)+'\n')
        result.write('Train loss:'+ str(train_loss_ep)+'\n')
        result.write('Validation loss:'+ str(valid_loss_ep)+'\n')
        ...
        train_loss = np.append(train_loss, [[train_loss_ep]])
        valid_loss = np.append(valid_loss, [[valid_loss_ep]])
...
class Dataset(object):
    def __init__(self, args, device, encoder_machine, channel_machine, 
        ...
        self.isi_coef = self.isi_coef_derivation(args.PW50)
        self.lor_di_coef = self.lorentzian_di_channel(args.PW50)
        
        print('The ISI coefficient is\n')
        print(self.isi_coef)
        print('The coefficient of dipulse Lorentzian is\n')
        print(self.lor_di_coef)
    ...
    def data_generation_train(self, prob, batch_size_snr):
        ...
        for i in range(batch_size_snr):
            ...
            for idx in np.arange(0, (args.snr_stop-args.snr_start)/args.snr_step+1):
                ...
                codeword_noisy_tmp = self.acgn_mis(np.concatenate((codeword, self.dummy_input_end[state]), axis=1), 
                                                   args.snr_start+idx*args.snr_step, 
                                                   self.isi_coef, self.lor_di_coef)
    ...
    def data_generation_eval(self, snr):
        ...
        codeword_noisy = self.acgn_mis(codeword, snr, self.isi_coef, self.lor_di_coef)
...
def train(data_class, prob, model, optimizer, epoch, device):
    ...
    return train_loss / args.num_batch
