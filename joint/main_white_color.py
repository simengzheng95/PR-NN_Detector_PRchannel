...
parser.add_argument('-batch_size_snr_train_white', type=int, default=30)
parser.add_argument('-batch_size_snr_train_color', type=int, default=30)
parser.add_argument('-batch_size_snr_validate_white', type=int, default=600)
parser.add_argument('-batch_size_snr_validate_color', type=int, default=600)
...
def main():
    ...
    for epoch in range(args.epoch_start, args.num_epoch):
        ...
        valid_loss, ber_awgn, ber_acgn = validate(data_class, prob, channel_dict, dummy_dict_start, 
        ...
        if (epoch >= args.eval_start and epoch % args.eval_freq == 0):
            result.write('-----[awgn] SNR[dB]:'+str(ber_awgn)+'\n')
            result.write('-----[acgn] SNR[dB]:'+str(ber_acgn)+'\n')
        else:
...
class Dataset(object):
    ...
    def data_generation_train(self, prob, bt_size_snr_white, bt_size_snr_color):
        '''
        training/testing data(with sliding window) and label
        output: float torch tensor (device)
        '''
        
        bt_size_white = int(((args.snr_stop-args.snr_start)/
                             args.snr_step+1)*bt_size_snr_white)
        bt_size_color = int(((args.snr_stop-args.snr_start)/
                             args.snr_step+1)*bt_size_snr_color)
        ...
        info_white = np.random.choice(np.arange(0, 2), 
                                      size = (bt_size_snr_white, info_length), 
                                      p=[1-prob, prob])
        
        info_color = np.random.choice(np.arange(0, 2), 
                                      size = (bt_size_snr_color, info_length), 
                                      p=[1-prob, prob])
        
        data_bt_white, label_bt_white = (np.zeros((bt_size_white, block_length)), 
                                         np.zeros((bt_size_white, 
                                                   args.eval_length+args.overlap_length)))
        data_bt_color, label_bt_color = (np.zeros((bt_size_color, block_length)), 
                                         np.zeros((bt_size_color, 
                                                   args.eval_length+args.overlap_length)))
                
        for i in range(bt_size_snr_white):
            codeword = (self.precoding(self.encoder_constrain(info_white[i : i+1, :]))
            ...
            for idx in np.arange(0, (args.snr_stop-args.snr_start)/args.snr_step+1):
                label_bt_white[int(idx*bt_size_snr_white+i) : 
                               int(idx*bt_size_snr_white+i+1), 
                ...
                data_bt_white[int(idx*bt_size_snr_white+i) : int(idx*bt_size_snr_white+i+1), 
                              :args.dummy_length_start] = codeword_isi_end[:, :args.dummy_length_start]
                data_bt_white[int(idx*bt_size_snr_white+i) : int(idx*bt_size_snr_white+i+1), 
                              args.dummy_length_start:] = codeword_noisy
        
        for i in range(bt_size_snr_color):
            codeword = (self.precoding(self.encoder_constrain(info_color[i : i+1, :]))
                        [:, :block_length - args.dummy_length_end])
            codeword_isi, state = self.e2pr4_channel(codeword)
            codeword_isi_end = np.concatenate((codeword_isi, 
                                               self.dummy_dict_end[state]), axis=1)
            ...
            for idx in np.arange(0, (args.snr_stop-args.snr_start)/args.snr_step+1):
                label_bt_color[int(idx*bt_size_snr_color+i) : 
                               int(idx*bt_size_snr_color+i+1), 
                               :] = (codeword[:, args.dummy_length_start:
                                              block_length-args.dummy_length_end])
                
                codeword_noisy = self.acgn(codeword_isi_end[:, args.dummy_length_start:], 
                                           args.snr_start+idx*args.snr_step)
                
                data_bt_color[int(idx*bt_size_snr_color+i) : int(idx*bt_size_snr_color+i+1), 
                              :args.dummy_length_start] = codeword_isi_end[:, :args.dummy_length_start]
                data_bt_color[int(idx*bt_size_snr_color+i) : int(idx*bt_size_snr_color+i+1), 
                              args.dummy_length_start:] = codeword_noisy
        
        data_bt = np.append(data_bt_white, data_bt_color, axis=0)
        label_bt = np.append(label_bt_white, label_bt_color, axis=0)
    ...
    def data_generation_eval(self, snr):
        ...
        codeword_awgn = self.awgn(codeword_isi, snr)
        codeword_acgn = self.acgn(codeword_isi, snr)
        
        data_eval_awgn = torch.from_numpy(codeword_awgn).float().to(self.device)
        data_eval_acgn = torch.from_numpy(codeword_acgn).float().to(self.device)
        label_eval = codeword
        
        return data_eval_awgn, data_eval_acgn, label_eval
...
def train(data_class, prob, model, optimizer, epoch, device):
    ...
    for batch_idx in range(args.num_batch):
        ...
        data_train, label_train = (data_class.data_generation_train
                                   (prob, args.batch_size_snr_train_white, 
                                    args.batch_size_snr_train_color))
...
def validate(data_class, prob, channel_dict, dummy_dict_start, 
    ...
    data_val, label_val = (data_class.data_generation_train
                           (prob, args.batch_size_snr_validate_white, 
                            args.batch_size_snr_train_color))
    ...
    ber_awgn = np.ones((1, int((args.snr_stop-args.snr_start)/args.snr_step+1)))
    ber_acgn = np.ones((1, int((args.snr_stop-args.snr_start)/args.snr_step+1)))
    
    if (epoch >= args.eval_start) & (epoch % args.eval_freq == 0):
        for idx in np.arange(0, int((args.snr_stop-args.snr_start)/args.snr_step+1)):
            data_eval_awgn, data_eval_acgn, label_eval = (data_class.data_generation_eval
                                                         (args.snr_start+idx*args.snr_step))
            dec_awgn = evaluation(data_eval_awgn, dummy_dict_start, dummy_dict_end_eval, 
                                  channel_dict, data_class, model, device)
            dec_acgn = evaluation(data_eval_acgn, dummy_dict_start, dummy_dict_end_eval, 
                                  channel_dict, data_class, model, device)
            ber_awgn[0, idx] = (np.sum(np.abs(dec_awgn.cpu().numpy() - label_eval))
                                /label_eval.shape[1])
            ber_acgn[0, idx] = (np.sum(np.abs(dec_acgn.cpu().numpy() - label_eval))
                                /label_eval.shape[1])
        print('Validation Epoch: {} [awgn] ber: {}'.format(epoch+1, ber_awgn))
        print('Validation Epoch: {} [acgn] ber: {}'.format(epoch+1, ber_acgn))
        
    
    return valid_loss.item(), ber_awgn, ber_acgn
