...
parser.add_argument('-loss_scaling', type=float, default=1)
...
class Dataset(object):
    ...
    def data_generation_train(self, prob, bt_size_snr_1, bt_size_snr_2):
        ...
        for i in range(bt_size_snr_1):
            ...
            for idx in np.arange(0, (args.snr_stop-args.snr_start)/args.snr_step+1):
                ...
                codeword_noisy = self.acgn(codeword_isi_end[:, args.dummy_length_start:], 
                                           args.snr_start+idx*args.snr_step, self.isi_coef_1)
        ...
        for i in range(bt_size_snr_2):
            ...
            for idx in np.arange(0, (args.snr_stop-args.snr_start)/args.snr_step+1):
                ...
                codeword_noisy = self.acgn(codeword_isi_end[:, args.dummy_length_start:], 
                                           args.snr_start+idx*args.snr_step, self.isi_coef_2)
    ...
    def data_generation_eval(self, snr):
        ...
        codeword_acgn_1 = self.acgn(codeword_isi, snr, self.isi_coef_1)
        codeword_acgn_2 = self.acgn(codeword_isi, snr, self.isi_coef_2)
        
        data_eval_acgn_1 = torch.from_numpy(codeword_acgn_1).float().to(self.device)
        data_eval_acgn_2 = torch.from_numpy(codeword_acgn_2).float().to(self.device)
        label_eval = codeword
        
        return data_eval_acgn_1, data_eval_acgn_2, label_eval
    ...
    if (epoch >= args.eval_start) & (epoch % args.eval_freq == 0):
        for idx in np.arange(0, int((args.snr_stop-args.snr_start)/args.snr_step+1)):
            ...
            ber_acgn_2[0, idx] = (np.sum(np.abs(dec_acgn_2.cpu().numpy() - label_eval))
                                  /label_eval.shape[1])
        print('Validation Epoch: {} [acgn] PW50_1 - ber: {}'.format(epoch+1, ber_acgn_1))
        print('Validation Epoch: {} [acgn] PW50_2 - ber: {}'.format(epoch+1, ber_acgn_2))
